...
from day26.mydao_suser import MyDao
from day26.mydao_survey import Sdao
from day26.mydao_sdetail import Sdetaildao
from day26.mydao_starget import Stargetdao
from day26.mymail import MyMail
from day26.mydao_sresult import MyDaoSresult
...
def sresult_ins_ajax():
    ...
    in_user_id = str(escape(session["in_user_id"]))
    up_user_id = str(escape(session["up_user_id"]))
    cnt = MyDaoSresult().myinsert(survey_id, s_seq, st_mobile, answer, in_date, in_user_id, update_id, up_user_id)
    msg = ""
    if cnt == 1:
        msg = "ok"
    else:
...
def sresult_upd_ajax():
    ...
    in_user_id = str(escape(session["in_user_id"]))
    up_user_id = str(escape(session["up_user_id"]))
    cnt = MyDaoSresult().myupdate(survey_id, s_seq, st_mobile, answer, in_date, in_user_id, update_id, up_user_id)
    msg = ""
    if cnt == 1:
        msg = "ok"
    else:
...
def sresult_del_ajax():
    survey_id = request.form["survey_id"]
    s_seq = request.form["s_seq"]
    cnt = MyDaoSresult().mydelete(survey_id, s_seq)
    msg = ""
    if cnt == 1:
        msg = "ok"
    else:
